...
class Trainer(object):
    ...
    def _build_pipeline(self):
        ...
        self.vectorizer = FeatureUnion([
            ('count_words', CountVectorizer(ngram_range=(1,2), analyzer='word')),
            ('tfidf', TfidfVectorizer(ngram_range=(1,2), sublinear_tf=True)),
            ('count_chars', CountVectorizer(ngram_range=(1,3), analyzer='char_wb'))
            ])
        self.feature_selection = SelectKBest(chi2, k=220000)
        self.classifier = MLPClassifier(early_stopping=True, 
    ...
    def predict(self, samples, label_only=False):
        ...
        if type(samples) == list:
            ...
            for string in samples:
                if label_only:
                    logging.debug(f'Classified example {counter}')
